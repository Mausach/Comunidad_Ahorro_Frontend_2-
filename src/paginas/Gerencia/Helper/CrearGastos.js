...
export const starCrearGastos = async (newGas,setRefreshData, navigate) => {
    try {
        const resp = await authApi.post('/admin/new-gasto', newGas);

        Swal.fire({
            title: "Gasto Registrado",
            text: "El gasto se ha sido cargado exitosamente.",
            icon: "success",
            background: "#f9f9f9",
        ...
        if (errores) {
            ...
            const errorMessage = error.response?.data?.msg || 'Error al registrar el gasto';
            Swal.fire({
                title: "ERROR",
                text: errorMessage,
                icon: "error",
