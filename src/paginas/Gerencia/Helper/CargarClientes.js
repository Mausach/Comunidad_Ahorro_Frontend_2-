...
export const CargarClientes = async (setUsers, navigate) => {
    try {
      const resp = await authApi.get('/admin/clientes'); // Petición al backend
  
      // Validar si la respuesta contiene un array de usuarios
      if (Array.isArray(resp.data.clientes)) {
        setUsers(resp.data.clientes); // Actualiza el estado con los usuarios
      } else {
